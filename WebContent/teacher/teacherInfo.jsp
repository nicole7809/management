...
<%@ page import="team.elite.management.ControlDAO" %>
	...
	<form action="teacherUpdate.jsp" method="post">
		...
		강사 이름 : <input type="text" name="name" value="<%=dto.getTeacher_name()%>" /> <br />
		강사 PW : <input type="password" name="pw" value="<%=dto.getPassword()%>" /> <br />
		Email : <input type="text" name="email" value="<%=dto.getEmail()%>" /> <br />
		Phone : <input type="text" name="phone" value="<%=dto.getPhone()%>" /> <br />
		사진 : <input type="text" name="admin_pic" value="<%=dto.getTeacher_pic()%>" /> <br />
