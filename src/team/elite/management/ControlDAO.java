...
public class ControlDAO {
	...
	public List getNotice(int start, int end)
		...
		try {
			conn = DataBaseConnection.getConnection();
			pstmt= conn.prepareStatement("select * from (select num,writer,subject,reg_date,content,ip,readcount,rownum r "+ 
	...
	//notice 글 수정 메서드
	public void updateArticle(NoticeDTO notice)
		throws Exception {
			try {
				conn = DataBaseConnection.getConnection();
	...
	// 노트 목록을 보는 것.
		public ArrayList getNote()
			...
			try {
				conn = DataBaseConnection.getConnection();
				pstmt= conn.prepareStatement("select * from note order by reg_date desc");
				rs = pstmt.executeQuery();
		...
		//note 글 수정 메서드
		public void updateArticle(NoteDTO note) // 오버라이딩
			throws Exception {
				try {
					conn = DataBaseConnection.getConnection();
					pstmt = conn.prepareStatement("update note set writer=?,subject=?,content=? where seqno=?");
					pstmt.setString(1, note.getWriter());
					pstmt.setString(2, note.getSubject());
					pstmt.setString(3, note.getContent());
					pstmt.setInt(4, note.getSeqno());		
					pstmt.executeUpdate();
				}catch(Exception ex) {
		            ex.printStackTrace();
		        } finally {
	...
	/*
	 * //SQL close 메서드 private void closeAll() { 
